// Shared verse-fetching logic used by both the API route and the tafsir page.
// Keeping it here avoids server components making HTTP requests back to their
// own server (which causes refresh loops in dev and fragile self-calls in prod).

let _tokenCache: { token: string; expiresAt: number } | null = null;
const _verseCache = new Map<string, { data: VerseResult; expiresAt: number }>();
...
async function generateTafsirSummary(tafsirHtml: string, verseTranslation: string): Promise<string> {
  const plainText = tafsirHtml.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim().slice(0, 3000);
...
export async function fetchVerse(verseKey: string): Promise<VerseResult> {
